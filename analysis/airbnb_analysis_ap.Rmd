...
```{r}
# due to computational limits, try sampling data first
n <- 40000
book_sample <- sample_n(book, size = n, replace = FALSE)
```
...
book_sample$ps_lr <- predict(lr, type = "response")
tapply(book_sample$ps_lr, book_sample$is_booked, mean)
```
...
book_sample$gender <- as.factor(book_sample$gender)
book_sample$first_device_type <- as.factor(book_sample$first_device_type)
book_sample$browser_group <- as.factor(book_sample$browser_group)
```
...
#summary(lm(is_booked ~ age + gender + treat + 
#                              first_device_type + browser_group, 
#                  data = book))
```
...
```{r, cache = TRUE}
# matching
matched_lr <- matching(z = book_sample$treat, 
                       score = book_sample$ps_lr, 
                       replace = TRUE)$cnts
```
